...
for ii = 1:numel(subjs)
    ...
    if ii < 10
        save(['CR_00' num2str(stimvars.(runs{3}).Subject(1)) '_stimvars+rt+acc.mat'],'-struct','stimvars');
    else
        save(['CR_0' num2str(stimvars.(runs{3}).Subject(1)) '_stimvars+rt+acc.mat'],'-struct','stimvars');
    end
end
